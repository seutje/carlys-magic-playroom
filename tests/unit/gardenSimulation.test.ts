...
describe("deterministic garden simulation", () => {
  it("reproduces serializable one-step and two-step definitions", () => {
    for (const steps of [1, 2] as const) {
      const first = generateGardenActivity("fixed-garden", steps);
      expect(first).toEqual(generateGardenActivity("fixed-garden", steps));
  ...
  it("produces the same safe growth for the same event sequence", () => {
    const definition = generateGardenActivity("water-sun", 2);
  ...
  it("bounds rapid and excess input without harming or resetting the plant", () => {
    const definition = generateGardenActivity("safe-garden", 1);
