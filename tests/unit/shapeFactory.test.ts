...
import { createShapeFactoryState, reduceShapeFactory } from "../../src/rooms/shapes/shapes.machine";
...
describe("deterministic shape factory", () => {
  ...
  it("pauses during drag, returns mismatches, reduces choices, and bounds rapid input", () => {
    const puzzle = generateShapePuzzle("machine-flow");
    ...
    let state = reduceShapeFactory(createShapeFactoryState(puzzle), { type: "INTRO_FINISHED" });
  ...
  it("processes once and recovers safely if processing is interrupted", () => {
    const puzzle = generateShapePuzzle("watchdog");
    const correct = puzzle.items.find((item) => matchesRule(item, puzzle.target));
    expect(correct).toBeDefined();
    if (!correct) return;
    let state = reduceShapeFactory(createShapeFactoryState(puzzle), { type: "INTRO_FINISHED" });
